...
export const checkAvailabilityApi = async (req: Request, res: Response) => {
  try {
    ...
    if (!roomId || !checkInDate || !checkOutDate) {
      return res
        .status(400)
        .json({ message: "room, checkInDate and checkOutDate are required" });
    }

    const overlappingBookings = await checkAvailability(
      roomId,
...
export const createBooking = async (req: Request, res: Response) => {
  try {
    const { userId, roomId, checkInDate, checkOutDate, numberOfGuests } =
      req.body;
    if (
      !userId ||
      !roomId ||
